...
<form id="frmEditarPersona" action="{{url('persona/editar')}}" method="post">
	...
	<div class="row">
		<div class="col-sm-3">
			...
			<div>
				<input type="date" id="dateFechaNacimiento" name="dateFechaNacimiento" class="form-control" value="{{substr($tPersona->fecha_Nacimiento, 0, 10)}}">
			</div>
		</div>
		<div class="col-sm-3">
			<label for="txtCorreoElectronico" class="control-label">Correo electrónico</label>
			<div>
				<input type="text" id="txtCorreoElectronico" name="txtCorreoElectronico" class="form-control" value="{{$tPersona->Correo_Electronico}}">
			</div>
		</div>
		<div class="col-sm-6">
			<label class="control-label">.</label>
			<div>
				<input type="button" class="btn btn-success" value="Guardar cambios" style="width: 100%;" onclick="enviarFrmEditarPersona();">
			</div>
		</div>
		{{csrf_field()}}
